...
static i32 round_i(double val)
{
  return i32(floor(val + 0.5));
}
//==================================================================================
void Chromagram::calculateChromagram()
{
  ...
  for (int n = 0; n < 12; n++)
  {
    ...
    for (int octave = 1; octave <= numOctaves; octave++)
    {
      ...
      {
        int centerBin = round_i((noteFrequencies[n] * octave * harmonic) / divisorRatio);
...
void Chromagram::calculateMagnitudeSpectrum()
{
  ...
  {
    fftIn[i].r = f32(buffer[i] * window[i]);
    fftIn[i].i = 0.0;
  }
  ...
  // compute first (N/2)+1 mag values
  for (i32 i = 0; i < (bufferSize / 2) + 1; i++)
  {
...
void Chromagram::downSampleFrame(std::vector<double> inputAudioFrame)
{
  ...
  for (int i = 0; i < inputAudioFrameSize; i++)
  {
    ...
    x_2 = x_1;
    x_1 = f32(inputAudioFrame[i]);
    y_2 = y_1;
    y_1 = f32(filteredFrame[i]);
  }
