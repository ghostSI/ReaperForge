...
std::vector<u8> Psarc::readPsarcData(const char* filepath)
{
    ...
    ASSERT(strncmp(reinterpret_cast<char*>(psarcData.data()), magicText, sizeof(magicText)) == 0 && "Invalid Psarc content");
...
static std::vector<u8> decryptPsarc(const u8* bytes, const u32 len)
{
    ...
    rijndael_setup(&ctx, 128, psarcKey);

    std::vector<u8> plainText(len);

    for (size_t i = 0; i < len; i+= 16)
    {
        rijndael_decrypt(&ctx, &bytes[i], &plainText[i]);
    }
