...
void Sprite::render(Sprite::Info &spriteInfo) {
    ...
    if (spriteInfo.rawTexture != nullptr) { // Load texture
        if (!spriteInfo.skipTextureCache) {
            const auto it = textureCache.find(
                    &reinterpret_cast<const u32 *>(spriteInfo.rawTexture)[spriteInfo.pixelOffset]);
            if (it == textureCache.end()) {
                GLuint tex;
                glGenTextures(1, &tex);
                textureCache.insert(
                        {&reinterpret_cast<const u32 *>(spriteInfo.rawTexture)[spriteInfo.pixelOffset], tex});

                glBindTexture(GL_TEXTURE_2D, tex);

                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, spriteInfo.rawTextureWidth, spriteInfo.rawTextureHeight, 0,
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, spriteInfo.rawTextureWidth, spriteInfo.rawTextureHeight, 0, GL_RGBA,
                         GL_UNSIGNED_BYTE,
                         &reinterpret_cast<const u32 *>(spriteInfo.rawTexture)[spriteInfo.pixelOffset]);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
